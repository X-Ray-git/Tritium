# 决策日志

## 2026-08-08：统计值必须可空解析，未知值显示 “—”

背景：多个页面用 `字段 ?? 0` 把缺失或解析失败的统计值伪装成真实 0。回答页底栏
在正文尚未加载时短暂显示 0；热榜把眼睛图标绑定到 `follower_count` 冒充浏览量；
`visit_count` 未加入 include 且与 `read_count` 的关系没有约定。

决策：统计值统一走 `lib/utils/count_format.dart` 的可空解析与格式化：
`parseCount` 只在字段存在且数值有效时返回 int，`formatCount` 对未知值输出 “—”。
浏览量按 `visit_count` 优先、`read_count` 兜底。热榜删除虚假浏览量指标，热度改用
`metrics_area.text`/`detail_text`，回答数使用 `answer_count`。

后果：任何上游字段缺失或解析失败都表现为 “—” 或隐藏指标，而不是错误数字；
“0” 只可能在接口明确返回 0 时出现。回答页底栏在正文加载前显示 “—”，加载后
再切换为真实数值。

合并复核补充：`NaN`、正负无穷和负数同样不是有效计数，必须拒绝；热榜字段需要
同时兼容 `target` 与卡片顶层的 `metrics_area/detail_text/label_area/card_label`，
不能因为只覆盖一种 envelope 又退回假指标。

## 2026-08-08：链接能力采用 URL 归一化 + 类型化路由

背景：旧 `ContentLinkService` 只解析带 scheme 的完整 URL，协议相对、站内相对
路径、`link.zhihu.com` 跳转、`zhihu://` 单复数、appview/oia 等都不完整；HTML
渲染用一个覆盖全部 `<a>` 的 TagExtension 把链接压扁成 `Text`，破坏了嵌套结构、
文字样式和选择能力。真实场景下正文里的站内链接几乎不可用，还会把站内目标误交
给系统浏览器。

决策：拆成三层。
1. **URL 归一化**：去除无效空白；支持 `//host` 协议相对、`/path` 与无前导斜线
   的站内路径；解包 `link.zhihu.com/?target=`，限制最大解包深度并用已访问集合
   防止循环重定向；拒绝 `javascript:`/`data:`/`file:` 等不安全 scheme。
2. **类型化解析**：覆盖 question/answer 组合路径、article/p、pin、people/org
   （机构复用用户页）、appview、oia/articles、zvideo/video 与 `zhihu://`
   单复数 deep link。
3. **导航策略**：有原生页的类型应用内打开；视频、话题、机构号、专栏等尚无原生
   页的知乎目的地和真正的外部网站交给系统浏览器；无效链接统一反馈，不静默失败。

HTML 侧不再用覆盖全部 `<a>` 的扩展：普通链接保留 flutter_html 的嵌套结构与
`onLinkTap` 回调（同时保留文字选择），只有评论“查看图片/动图”链接使用专用
缩略图扩展；`#anchor` 由 flutter_html 内置页内定位处理，找不到目标时不再交给
浏览器或链接服务；没有 `href` 的蓝字不伪装成可点击。

后果：正文、评论、楼中楼、历史记录与 Android 外部 Intent 全部复用同一套归一化
与路由；新增原生内容类型时只需扩展类型化解析与路由表。

合并复核补充：给无 scheme 的链接补 `https://www.zhihu.com/` 时必须保留原始
query 和 fragment，否则 `link.zhihu.com?target=...` 会在归一化阶段丢失目标；所有
依赖 ID 的站内类型只接受纯数字 ID，避免把畸形路径送进原生页。系统浏览器启动也
必须捕获平台异常并显示统一错误反馈。

## 2026-08-08：正文 HTML 语义组件与稳定布局共存

背景：正文为防阅读进度跳动采用一次布局全部语义块（`SliverToBoxAdapter +
Column`）的稳定高度模型，但渲染器把所有链接压成 `Text`，行内代码没有正确的
baseline 对齐，代码块是默认浏览器样式，表格会横向溢出。

决策：在保持稳定布局不变的前提下，用 flutter_html 的扩展体系替换缺语义：
行内代码通过 `WidgetSpan`（`PlaceholderAlignment.baseline` + alphabetic）渲染为
半透明圆角胶囊，`<pre>` 用专用扩展渲染为单容器代码块（8px 圆角、细边框、横向
滚动、复制按钮带 1.2 秒勾选态），`<pre><code>` 不会出现两层背景；引用块、表格
（宽表横向滚动）、列表、分割线、正文图片占位与错误态统一视觉。文本选择在页面
级接入 `SelectionArea`：回答页包在整个 PageView 外层（SelectionArea 是 PageView
的祖先时不会抢走横滑手势，这是 Flutter 文档化的手势仲裁行为），文章/想法/问题
描述/评论则包在滚动视图外层。

后果：稳定布局（阅读进度、标题时机、评论预加载）不受影响，同时获得可对齐
Fourier（原 Auto Folo）的正文语义和系统文本选择。

合并复核补充：宽表单元格宽度沿用 Fourier 的 112–180dp 约束，并以最多四列参与
可视宽度计算，避免四列以上时产生负约束；代码块保留源码首尾空白。专栏封面比例
统一使用 Flutter 的宽/高语义，优先接口尺寸，缺失时才解析实际图片尺寸；尺寸监听
不释放由共享 ImageStream 管理的 `ImageInfo`。

## 2026-08-08：首页三页 PageView 显式保活

背景：主导航原来用 `_FadeIndexedStack` 淡入淡出，无法左右滑动；直接换
PageView 后，离屏页面默认会被回收重建，重新请求数据并丢失滚动位置。

决策：三个主页用 `AutomaticKeepAliveClientMixin` 的包装显式保活；离屏页面用
`TickerMode` 暂停动画（保留状态），避免加载指示器空转；底部导航点击用
`animateToPage` 动画切页，编程式切页期间抑制 `onPageChanged` 的中间页同步，
滑动落定（`ScrollEndNotification`）后再以实际停靠页为准；重复点击当前导航
按钮仍触发既有滚回顶部语义。

后果：推荐/热榜不重复请求、不丢滚动位置；设置页的 Switch 等横向控件作为
PageView 的子级保留自己的手势（子级先进入手势竞技场）。

## 2026-08-08：回答分页所有权与 cursor 防循环

背景：回答页只拿到问题页已经加载的 ID 快照，把这一批的末尾误认为全部回答
末尾；从推荐页等只传入单个回答的入口进入时没有可继续的分页游标。

决策：建立 `AnswerPager` 种子/状态模型（questionId、answerIds、nextUrl、
sortBy），从问题页进入时把当前 ID 列表、下一页游标和排序方式一并传入，回答页
接管后续分页；单回答入口先获取所属问题第一页并把当前回答保留在列表最前。
追加页按 ID 去重；空页、重复 cursor、已消费 cursor 与服务器 `is_end` 都视为
无进展并停止自动请求，加载失败保留可重试状态；追加数据不改变既有条目的索引，
PageController 不跳页；滑动进入加载占位页不触发“已切换回答”的振动。

后果：回答横滑可以一直滑到问题真正的末尾；分页失败不会永久卡死在假末尾。

合并复核补充：当前回答无论是否已出现在补齐页中都必须移到种子列表首位；cursor
只有请求成功后才记为已消费，失败时保留以便重试。刷新代次已经变化的旧响应直接
丢弃，不得回写新一代请求的加载锁。

## 2026-08-08：统一反馈入口替代散落的状态提示

背景：`Get.snackbar`、`Get.rawSnackbar`、`SnackBar` 与仅状态用的
`SmartDialog.showToast` 混用，风格不一致，图片保存成功也没有成功动画。

决策：新增 `TritiumFeedback`（info/success/warning/error），顶部居中、SafeArea
内显示、毛玻璃半透明胶囊、状态色圆形图标、从顶部滑入淡入退出反向动画、不拦截
页面交互；全部状态反馈迁移到该入口，加载遮罩仍由 SmartDialog 自行控制。

## 2026-07-19：固定产品边界

背景：旧工程包含关注、点赞和较复杂的回复入口，与当前阅读客户端定位不符。

历史决策：主导航曾仅保留内容与设置；现已将内容内的推荐/热榜提升为主导航入口。继续删除远端点赞、关注、回复、收藏、私信、发现和 AI
功能，公开数量保持只读展示。回复采用单层弹层查看。

后果：新增功能必须先确认不突破只读边界，不要恢复已删除的交互入口。

## 2026-07-19：固定品牌与包名

决策：品牌色固定为 RGB(57, 97, 255) / `#3961FF`，包名改为
`io.github.xraygit.tritium`。品牌色不从图标或系统动态生成。

后果：旧包无法被新包覆盖安装；图标需要另行适配，但不能反向改变应用主题色。

## 2026-07-19：参考工程职责分离

决策：Hydrogen 只参考功能模块和知乎接口，Fourier（当时名 Auto Folo）只参考设计、维护架构与发布
流程。Tritium 不复制其产品业务源码，也不修改参考工程；经确认需要完全一致的基础交互
可以复用 Fourier 已受控维护的上游开源实现，并在 Tritium 内独立测试和维护。目前
下拉刷新状态机属于这一例外。

## 2026-07-19：发布必须可追溯

决策：版本发布使用 `scripts/release.sh` 创建版本提交和 annotated tag；GitHub Actions
校验 tag 与 `pubspec.yaml`、执行分析和测试、注入正式签名并发布 APK。普通提交和未
签名构建不创建 GitHub Release。

## 2026-07-19：登录诊断不得泄露凭据

决策：登录检查点仅在 Debug 构建输出安全事件，隐藏查询参数和 Cookie 值；WebView
结束回调缺失时允许进度或 12 秒超时撤下遮罩。

后果：排障日志可以直接用于判断状态机，但仍不得要求用户提供 Cookie 或完整 URL。

## 2026-07-19：与 Fourier（当时名 Auto Folo）共享 Android 签名

决策：Tritium 的 GitHub Actions 复用 Fourier 的 Android keystore，通过 Tritium
仓库独立的四项 Secrets 注入；证书和密码不进入源码仓库。

后果：两款应用的签名身份和密钥轮换相互关联，密钥泄露会同时影响两者。共享签名不
改变两个应用的包名、数据目录和更新关系。

## 2026-07-22：发布流程参考不等于版本盲从

决策：版本提交、annotated tag、签名构建和 GitHub Release 的阶段设计继续与 Auto
Folo 对齐；Flutter 与 GitHub Actions 版本需要在 Tritium CI 中独立验证。参考工程中
尚不存在的 action tag 不得直接同步。

## 2026-07-28：参考工程采用本地只读布局

决策：Hydrogen 长期放置在 `reference/hydrogen/`，作为被 Tritium 根仓库忽略的独立
Git 仓库。参考目录不使用 submodule，不进入 Tritium 的依赖、构建、测试或发布输入。

后果：维护者可以从固定路径核对实现，同时不会误提交 Hydrogen 的历史、构建产物或
签名文件。新克隆需要参考时自行放入该目录；目录缺失不得影响 Tritium 正常维护。

## 2026-07-28：阅读能力保持本地、链接能力分层兜底

决策：参考 Hydrogen 补齐 Android 外部知乎链接接收与本地阅读历史，但历史和正文
进度只写入 Tritium 本机，不同步知乎服务器。Tritium 已有原生只读页的链接内部
跳转；其他知乎内容统一转换为 HTTPS 并由系统浏览器打开。

决策：参考 Fourier 增加 1 像素阅读进度条，但以正文末尾而非整页最大滚动范围为
终点，避免评论懒加载让进度倒退。文章、回答和想法复用同一阅读会话组件。

后果：新增原生内容类型时必须同时补充链接解析、历史类型和标准 URL；不得借由链接
能力引入关注、点赞、评论发布等远端写操作。

## 2026-07-28 至 2026-07-29：导航玻璃和启动图标不得近似适配

决策：主导航完整复用 Fourier 当前移动端玻璃表面的连续圆角、渐变、方向性边缘
高光及内外阴影层次，不再用普通边框近似。Android 图标与 Fourier 一样只提供
静态 `mipmap`，从源图直接做无内边距圆角裁剪；不再提供会被启动器二次裁切和缩放
的 Adaptive Icon XML。

排查过程：源文件 `assets/images/logo/icon.png` 本身符合预期，但 Android 设备显示
类似“只取中央约 0.1…0.9 再撑满”的效果。这个范围只是用户帮助定位问题的近似描述，
曾被错误当成精确映射并生成固定 10% 反向补偿；该方案随后立即撤销。对比 Fourier
最终资源后确认，真正差异是 Tritium 的 `mipmap-anydpi-v26/ic_launcher.xml` 会让
Android 8+ 改走 108dp Adaptive Icon 图层，而 Fourier 只有静态 mipmap。因此最终
删除 adaptive XML、前景、背景和专用颜色资源，而不是保留任何经验缩放。

后果：以后更换 `assets/images/logo/icon.png` 后运行
`scripts/generate_logo_assets.py` 和 `dart run flutter_launcher_icons`，不得根据
单一启动器的近似显示范围增加经验缩放，也不得重新引入 adaptive 前景安全区。

## 2026-07-29：正文进度稳定性优先于变量高度惰性构建

背景：多图长正文通过 `SliverList` 按需创建语义块时，尚未出现的块与图片真实高度
会持续修正评论锚点位置。阅读进度以该锚点作为正文终点，因此分母变化会造成进度条
前后跳动；这与 Fourier 曾经依赖动态 `maxScrollExtent` 时的问题属于同一类。

决策：文章和回答共用的 `ChunkedHtmlSliver` 保留后台解析、语义分块和短块合并，但
改用 `SliverToBoxAdapter + Column` 一次布局所有块。图片继续使用显式比例或合理
预估占位，不通过进度动画掩盖几何变化，也不强制未知比例图片永久使用估算高度。

后果：超长正文会一次创建更多 Widget，但正文终点在开始阅读前即可确定；不得在
缺少稳定高度模型的情况下恢复变量高度 `SliverList`。

## 2026-07-29：缩略跟评保留表情语义

背景：完整楼中楼使用 `CustomHtml`，而根评论内的缩略跟评曾用正则删除全部 HTML
后直接显示纯文本，导致 `[捂脸]` 等文本表情和 `<img class="emoji">` 都无法加载。

决策：缩略跟评使用非交互的紧凑 HTML 预览，只保留文本、基础强调和知乎表情；普通
图片附件仍省略，链接不接管点击，整个预览继续用于打开只读楼中楼。

## 2026-07-29：正文进度以视口下边缘到达末尾为完成

背景：旧公式使用当前 offset 除以“正文末尾对齐视口顶端”的 offset。正文之后的评论
很短时，页面最大滚动范围可能不足以让视口顶端到达正文末尾，因此进度永远无法满格。

决策：通过 `RenderAbstractViewport.getOffsetToReveal(bodyEnd, 1)` 取得正文末尾对齐
视口下边缘的实际滚动终点，并在该终点显示 100%。正文末尾初始已经位于视口内时，
可用正文滚动距离为零，布局完成后直接显示 100%；不隐藏进度条，也不要求用户执行
没有意义的滚动。

后果：很短的正文会在打开后满格；文字虽然很短但封面、标题或作者信息让正文末尾仍
在屏幕外时，仍需滚动到末尾进入视口。恢复位置与进度保存使用同一终点，评论加载和
评论长度变化不参与计算。

## 2026-07-31：评论区改为自动加载下一页

背景：评论组件 `InlineCommentWidget` 以 `childCount: 1` 的 `SliverList` 形式内联
嵌在回答/文章/想法的正文滚动视图里。它只维护自身的分页状态（`_nextUrl`），没有
与父级滚动事件挂钩，因此在评论还有下一页时，只能渲染一个“查看更多评论”按钮，要求
用户手动点击才能继续加载。这是实现缺口而非有意的产品取舍——对照推荐流
（`RecommendPage`）已经通过监听 `extentAfter` 做了自动加载。

审查中否决过一版实现：它在评论组件内部包 `NotificationListener`，但父滚动视图是
组件的祖先，滚动通知不会向后代传播；同时它把
`RenderAbstractViewport.getOffsetToReveal(...).offset` 当作哨兵相对屏幕顶部的
坐标。该值实际是同一滚动轴上的绝对目标 offset，长文章即使已经滚到评论区也可能
一直拿数千像素的绝对值与屏幕高度比较，导致自动加载只在短内容中偶然生效。那一版
纯函数测试没有挂载真实父滚动视图，因此虽然全部通过，却没有覆盖核心链路。

最终判断：`InlineCommentWidget` 自身位于外层 `Scrollable` 的后代，可以在
`didChangeDependencies` 通过 `Scrollable.maybeOf(context, axis: Axis.vertical)`
取得祖先 `ScrollPosition`，正确注册/解除 listener。尾部仍放置 `GlobalKey` 哨兵，
但计算统一使用滚动坐标系：

`哨兵绝对 reveal offset <= 当前 pixels + viewportDimension + preloadExtent`

为何不破坏正文进度稳定性：正文进度锚点（`commentsKey`）位于评论列表**之前**，
评论加载只会让下方内容变长，不改变正文终点的滚动位置；因此自动加载评论不会触发
“阅读进度条随评论懒加载跳动”这一已记录问题。

决策：评论区尾部从固定按钮改为以自动加载为主、保留可点击兜底的三态：
- **空闲**：仍展示“查看更多评论”，兼作几何哨兵和可访问入口。
- **加载中**：转圈。
- **失败**：原来加载更多失败仅弹一次 SnackBar 就消失，改为在尾部展示“加载失败，
  点击重试”的退化态，用户可恢复。

触发逻辑抽成纯函数 `shouldAutoLoadMore`（`lib/utils/comment_auto_load.dart`），
默认 `preloadExtent = 720`，与正文评论预加载距离一致。首次页面成功和每次追加成功
后在下一帧补检，覆盖短正文或一页评论不足以填满视口的情况；真实父滚动 listener
覆盖长正文继续下滑。该阈值保持内部实现细节，不增加面向用户的技术设置。

分页请求捕获本次 `nextUrl`，通过加载锁防止并发；成功页按评论 ID 去重，并记录已经
消费的游标。追加页没有新评论、返回同一游标或回到已消费游标时立即停止，避免自动
模式不断请求和追加重复数据。加载更多错误只显示尾部重试；重新排序、首屏重载或成功
请求会清理旧错误。通用 `TritiumGlassSheet` 同时改用真实 `Material` 承载背景和
水波纹，避免排序 `ListTile` 的 ink 被装饰层遮挡。

后果：评论滑到末尾会自动拉取下一页，无需手动点击；按钮仍作为可访问和异常兜底。
评论目前仍在单个 `Column` 中保留所有已加载项，这是既有架构的内存风险；本轮不为
没有复现的性能问题扩张成 Sliver 重构，超长评论区如出现掉帧再单独设计虚拟化。

## 2026-07-31：推荐重复分层处理，正文进度条承担定位

背景：Hydrogen 将首页刷新重复、数据不全和无法稳定判断新内容列为长期难题。
Zhihu++ 已增加稳定身份去重、多推荐来源和推荐反馈，但其 2026 年仍有同页重复报告；
最新反馈修正也明确证明 `/lastread/touch` 是推荐信号而非“永久排除已看内容”的
契约。因此不能把服务端重复全部包装成客户端可保证解决的问题。

产品边界：本轮所说“推送/推荐阅读”仅指首页推荐流，不引入 Android 系统通知、后台
推送、内容屏蔽、跨启动过滤或长期“看过不再推荐”设置。Tritium 的许可证和参考工程
继承关系仍需另行研究；本轮只参考可观察行为、协议证据和测试思路，以 Dart 独立
实现，不复制 Zhihu++ 的 AGPL 源码。

决策分为两层：

1. 对响应中能够验证的内容类型与 ID 建立稳定身份，在当前应用进程内统一处理首屏、
   分页和刷新去重。刷新整页替换为本会话新内容；如果没有新内容，保留当前列表并
   提示，不无限追页。空新增页、同一 `nextUrl` 和回到已消费内容都停止。
2. 推荐页处于当前主导航且列表停稳后，只收集真实屏内卡片。回答、文章和问题以
   已验证的 API `targets` 格式批量上报触达，实际打开时上报已读；未知卡片、想法和
   视频不伪造目标。失败留待本会话后续事件重试，载荷不记日志。

这层会话身份集合不写入 Hive，进程退出即清除。它修复客户端自己能够确定的重复，
但不承诺知乎在新进程或新会话中不再推荐相同内容。

同时将顶部 1 像素正文进度条升级为定位器：视觉仍贴住 AppBar，下方提供 24dp 隐形
横向拖动区域，拖动时加粗并显示百分比。映射继续使用“正文末尾到达视口下边缘”为
100%，评论不进入范围；正文不足一屏时保持 100% 且禁用拖动。回答横滑页面由当前
停稳回答独立响应定位，邻页不会被改动。

## 2026-08-12：内容身份而非路由名决定是否允许导航

背景：正文内问题链接的初次修复只对 `/question` 设置 `preventDuplicates: false`，
组件测试也只覆盖问题→问题。真机采集表明用户点击的许多“问题链接”实际包含回答
路径；触摸、HTML 和解析均成功，但 `/answer`→`/answer` 被 GetX 默认按路由名去重。
专栏链接之所以正常，只是它从 `/answer` 切换到了 `/article`。

决策：`ContentLinkService` 管理的原生内容一律允许同名路由入栈，具体内容身份由
路由参数中的 ID 决定。该规则覆盖回答、问题、专栏文章、想法和用户，避免逐类型打
补丁。直接 `Get.toNamed` 的现有入口经审计均发生在不同路由之间，不扩大修改范围。

真机同时证明 SelectionArea 的 pointer 命中兜底与 flutter_html 原生 recognizer
可能对同一次单击各回调一次。不能仅放开 GetX 去重，否则会连续压入两个相同页面；
也不能把目标锁定到新页面退出，否则用户稍后再次访问相同内容会被误拦截。最终按
`kind + id` 合并 500ms 内的重复分发，窗口结束后恢复正常导航。

后果：回答→回答等同类型链接可用，单击只进入一页，正常返回栈不变；测试必须同时
覆盖同类型不同 ID、同一时刻双回调和窗口结束后的再次访问。Android Debug 使用独立
包名与正式版并存，后续真机诊断无需牺牲正式版阅读历史。

## 2026-08-17：Flutter、锁文件与 CI 必须作为同一兼容性基线

背景：本机 Flutter 从 3.44.6 升级至 3.47.0 后，Dart 版本变为 3.13.0。现有
`pubspec.lock` 在 `--enforce-lockfile` 模式下无法满足新版 Flutter，必须随 SDK 更新
`intl`、`matcher`、`meta`、`test_api` 和 `vector_math`。若只提交新版锁文件而继续让
CI 使用 Flutter 3.44.6，发布构建可能反向无法解析依赖。

排查还发现 4 项手势测试在新版 SDK 下稳定失败，但普通 `PageView`/`ListView` 探针
正常；把页面横滑改为带速度的 `fling`、精确滚动改为分帧 `timedDrag`、原始指针事件
改为递增时间戳后，相关行为和全套测试均恢复。这说明失败来自零时长合成手势的测试
假设，不应通过修改生产手势逻辑来迎合测试。

决策：工程、锁文件和发布工作流统一固定在 Flutter 3.47.0、Dart 3.13.x 与 JDK 21；
升级 SDK 时三者必须在同一兼容性提交中更新。页面停稳测试使用 `fling`，几何阈值使用
`timedDrag`，状态机的原始指针测试使用分帧时间戳。

Flutter 3.47 对 Gradle 8.14、AGP 8.11.1 和 Kotlin 2.2.20 给出的未来弃用预警另立
构建链升级任务；当前版本仍可构建，不在本次 SDK 适配中同时跨越三个 Android 大版本，
以免混淆 Flutter 回归和 Android 构建回归。
