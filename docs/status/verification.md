...
- 发布版本为 `v0.4.4+10`；上一版 `v0.4.3` 的 Linux 发布工作流已成功完成。
- 本地静态分析与差异检查通过；全套测试再次因 Xcode 未接受许可、objective_c
  原生 hook 无法获取 SDK 路径而在测试启动前失败。
- 本次使用既有 Linux Release CI 完成完整测试、签名构建与发布；测试成功是发布
  必要条件，本地测试未通过的环境限制不等同于回归测试成功。

## 2026-09-17：表情标签属性泄漏修复

- 用户发现评论表情旁出现编码。最小 HTML 解析探针确认：原始字符串替换会把
  `img alt="[微笑]"` 中的表情名也替换为 img 标签，导致 class/style 等属性成为
  可见文本；9 月 14 日分段渲染再次执行替换，使纯文本表情也触发该问题。
- 修复为只遍历 DOM 文本节点转换已知表情，保留已有图片和链接属性，内部片段
  直接复用已处理内容；代码、公式与原始文本标签不转换。影响范围包含完整评论、
  楼中楼、文章/回答正文、问题描述和想法；缩略跟评的独立 DOM 实现无此缺陷。
- 新增四项回归用例：连续表情与未知表情、已有图片和链接属性、列表引用嵌套、
  代码与 HTML 转义文本。`dart analyze` 通过。
- Widget 测试尚未进入执行：本机 Xcode 未接受许可，objective_c 原生构建 hook
  获取 SDK 路径失败；尝试为测试进程指定 Command Line Tools 仍失败。需本机完成
  Xcode 许可设置后复跑表情、选择、链接与评论测试；不能将本次结果记为测试通过。

## 2026-09-14：Fourier GUI 历史审计与可复现机制修复

- 审计 Fourier 从早期到当前的 GUI 修复历史，并逐项对照 Tritium；没有迁移已明确
  放弃的半透明前端，而是用 Android Widget 测试复现文字选择、parser 稳定性、空块
  和主页 PageView 重建边界。
- 正文段落、标题、列表、嵌套引用和行内代码现在进入真实可选择文字流；普通链接使用
  单一原生回调，移除会造成双回调的 RenderParagraph 命中兜底。新增测试覆盖 Android
  长按选择、嵌套引用、HTML 转义、父级无关 rebuild 保持 parser、点击/拖动/长按链接。
- HTML 后台分块过滤格式空块；正文图片加入 1/2/4 秒有限重试和 4 秒后静止的加载
  标记；图片 URL 变化会清理旧重试状态。主页 PageView 移出导航索引的响应式重建范围。
- 发布 CI 对齐 Fourier 的已验证选择补丁机制：固定 Flutter 3.47.0 精确 revision，
  构建前幂等应用仓库补丁。本机脚本校验结果为 SDK 已包含兼容选择支持。
- `dart analyze`：通过，无诊断。
- 正文选择/链接/分块/代码/图片布局/主页导航定向测试：38 项通过。
- 全套测试第一次执行发现回答分页测试依赖正文恰好拆成独立 Text Widget；生产行为正常，
  测试断言已改为匹配合并后的可选择文本流。`flutter test` 最终 147 项全部通过。
- `flutter build apk --debug` 与 `flutter build apk --release`：均通过；Release APK
  67.3MB。构建仍仅报告已记录的 Gradle 8.14、AGP 8.11.1、Kotlin 2.2.20 后续升级
  预警，没有使用跳过依赖验证参数。
- 补丁脚本 `bash -n`、`git diff --check`：通过。

真机仍需检查系统选择工具条、复制结果、异步评论更新期间选择保持，以及断网/弱网下
图片有限重试；已同步到待办与真机验收清单。
