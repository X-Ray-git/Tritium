...
class Zse96Encrypt {
  ...
  static ({String rewrittenUrl, Map<String, String> headers})
  generateSignHeadersWithUrl({required String url, String? cookies}) {
    // 1. 获取 d_c0
    String dc0 = '';
    if (cookies != null) {
  ...
  static String _b64encode(String md5Hex, {int device = 0, int seed = 63}) {
    ...
    for (int i = combined.length - 1; i >= 2; i -= 3) {
      final b0 =
          combined[i] ^ (_unsignedRightShift(58, 8 * (shiftCounter % 4)));
      shiftCounter++;
      final b1 =
          combined[i - 1] ^ (_unsignedRightShift(58, 8 * (shiftCounter % 4)));
      shiftCounter++;
      final b2 =
          combined[i - 2] ^ (_unsignedRightShift(58, 8 * (shiftCounter % 4)));
      shiftCounter++;
  ...
  static List<int> _transformBlock(List<int> data) {
    ...
    for (int r = 0; r < 32; r++) {
      ...
      final temp =
          (words[r + 1] ^ words[r + 2] ^ words[r + 3] ^ zkVal) & 0xFFFFFFFF;
